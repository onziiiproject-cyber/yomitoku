...
- ミスグレー（speaker: "gray"）: 新人記者（猫キャラ）。素朴な「そもそも」な疑問を投げかける。猫キャラだが語尾に「にゃ」などは付けず、普通の丁寧語で話す。
...
- 締めのサービス紹介パートは別途固定で追加するので、この台本には含めない。最後はミスグレーの「勉強になりました」等の一言で軽く締めてよい（「続きは編集部で」等のフレーズは書かない）
- 事業所固有の対応や助言は含めない
...
登場人物:
- ゴリ編集長（speaker: "gori"）: 介護保険制度に詳しいベテラン編集長。頼れるがフランクな口調。
- ミスグレー（speaker: "gray"）: 新人記者（猫キャラ）。委員の発言や議論の流れを気になったところから聞いていく。猫キャラだが語尾に「にゃ」などは付けず、普通の丁寧語で話す。

添付した議事録PDF（${sessionLabel}）のうち、「${themeTitle}」というテーマに関する部分を中心に、実際にどんな意見が出たか・どんなやり取りがあったかを、ラジオのように振り返る台本を作成してください。

条件:
- 【必須】冒頭は「①いつ・どの会議の話か」→「②その回がどんな空気だったか」の順で組み立てる。この順番を守ること。
  ①まず1〜2行目で、議事録に書かれている開催年月日と回次、今日扱うテーマを示す（例: ミスグレー「今日はどの会議の話ですか？」→ ゴリ編集長「2026年7月13日に開かれた第260回の介護給付費分科会だ。今日はその中の訪問介護の議論を振り返る」）。音声だけを聴く人が、いつの議論なのか分からないまま聞き進めることがないようにする
  ②その直後に、**その回がどんな空気の議論だったかを2人の感想のやり取りで伝える**。ミスグレーが議事録を読んだ第一印象を投げ、ゴリ編集長がそれを受けて言い換えるか、少し訂正して実際の温度感を示す。例:「訪問介護の話、すごく盛り上がってたみたいですね」→「盛り上がった、というか、かなり緊張感のある議論だったな」。この「思っていたのと少し違う」という差分が、続きを聴きたくさせる
- 冒頭で使う感想は、必ず議事録の実際の中身に裏打ちされたものにする（紛糾したなら紛糾したと、淡々と進んだなら淡々と進んだと言う）。盛り上がっていない回を盛り上がったことにするなど、実態と違う煽りは書かない
...
- 委員・参考人・事務局の個人名（氏名）は一切出さない。「ある委員から」「利用者側の委員は」「連合の委員が」「事務局の推進課長は」のように、立場・所属団体・役職で表現する（音声では誰の発言かの特定より、どういう立場からの意見かが伝わることを優先する）
- ミスグレーが聞き役になり、ゴリ編集長が実際の議論を要約・解説しながら紹介していく掛け合い形式
- 台本は28〜36行程度、合計1800〜2500字程度（6〜9分程度の尺を想定）
- 終盤で「令和9年度の介護報酬改定に向けて、この論点が今後どう進んでいきそうか」の見通しに触れる。事務局が示した論点の立て方、次回以降に提出を求められたデータ、意見が割れていて調整が要りそうな点などから読み解く。これは議論の材料から導ける範囲にとどめ、改定内容や結論を断定・予測しない（「〜という方向で議論が続きそう」「次はこのデータを踏まえた議論になりそう」といった書き方にする）
